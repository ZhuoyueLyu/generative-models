...
class ACAI_autoencoder(Model):
    def __init__(self, hidden_dim=500, latent_dim=2):
        ...
        self.encoder = Sequential([
            Dense(hidden_dim, activation='relu', input_shape=(midi_shape,)),
            Dense(latent_dim)
        ])
...
if __name__ == "__main__":
    ...
    # Getting data 
    data = tf.cast(get_midi_data(), tf.float32)
    ...
    alpha = tf.random.uniform((data.shape[0], 1), 0, 0.5)
